	...
	var JoinGameView = (function makeJoinGameView(){
		...
		function makeJoinRactive(game){	
			...
			var nationMap = {
				...
				"white":"America", 
				"brown":"Portugal"};
			//TODO make these strings appear instead of color names.
			...
			var colors = colorListString.toLowerCase().split(",").map(
				function(color){ return {nation:nationMap[color],name:color,taken:(!(!usedColors[color]))};}
			);
			...
			var template = ""
				...
				+ 		'<input type="radio" name="color" value="{{.name}}" disabled={{.taken}}><span>{{.nation}}</span>'
				+ 		'</label>{{/colors}}' 
				+   '</div></form>';
